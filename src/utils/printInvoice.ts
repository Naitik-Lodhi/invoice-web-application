...
export const printInvoice = async (
  ...
  if (companyInfo?.companyID) {
    const storedLogo = localStorage.getItem(
      `company_logo_base64_${companyInfo.companyID}`
    );
    if (storedLogo) {
...
export const printMultipleInvoices = async (
  ...
  console.log(`🖨️ Printing ${invoices.length} invoices`);

  // Get logo once for all invoices
  let logoBase64 = "";
  let useFallback = true;
                ...
                .map(
                  (item, idx) => `
                    <tr>
                      ...
                      <td>${item.name}</td>
                      <td class="text-center">${item.quantity}</td>
                      <td class="text-right">${companyCurrency}${(item.rate || 0).toFixed(2)}</td>
                      <td class="text-center">${item.discountPct || 0}%</td>
                      <td class="text-right">${companyCurrency}${(
                    item.quantity * item.price
                  ).toFixed(2)}</td>
                    </tr>
  ...
  const printHTML = `
    ...
    <body>
      ...
      <script>
        console.log("📄 Print window loaded for ${invoices.length} invoices");
        window.onload = function() {
          setTimeout(() => {
            window.print();
          }, 500);
  ...
  if (printWindow) {
    printWindow.document.write(printHTML);
    printWindow.document.close();
    console.log("✅ Print window opened for multiple invoices");
  } else {
    console.error("❌ Failed to open print window");
    alert("Please allow pop-ups to print the invoices.");
  }
};
