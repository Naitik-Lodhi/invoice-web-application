...
import {
  IconButton, InputAdornment, TextField, Typography, Box, LinearProgress, List, ListItem, ListItemIcon, ListItemText
} from '@mui/material';
...
interface PasswordStrength {
  id: number; // 0: Too weak, 1: Weak, 2: Medium, 3: Strong
  value: string;
  contains: string[];
  length: number;
}
...
interface PasswordFieldProps<T extends FieldValues> {
  ...
  label: string;
  placeholder?: string;
    showStrengthMeter?: boolean; // New optional prop
    maxLength?: number;
}
        ...
        <Box sx={{ width: "100%", mb: 2.5 }}>
          ...
          />
           ...
           {showStrengthMeter && field.value && strength && (
              <Box sx={{ mt: 1.5 }}>
                <LinearProgress variant="determinate" value={progress}  sx={{ height: '6px', borderRadius: '3px' }}/>
                <Typography variant="caption" color={color === 'error' ? 'error.main' : 'text.secondary'} sx={{ mt: 0.5, display: 'block' }}>
                ...
                <List dense sx={{ p: 0, mt: 1 }}>
                  {criteria.map((item) => {
                    ...
                    return (
                      <ListItem key={item.id} sx={{ p: 0, color: isMet ? 'success.main' : 'text.secondary' }}>
                        <ListItemIcon sx={{ minWidth: 28 }}>
                          {isMet ? <CheckCircle fontSize="small" color="success" /> : <Cancel fontSize="small" />}
                        </ListItemIcon>
